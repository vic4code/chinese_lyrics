...
filelist = [['data/jay.mp3', 'data/jay.lrc', 'data/jay.ass']]
...
if __name__ == '__main__':
    ...
    filelist = []
    audio_path = os.path.join('data', 'matched_files')
    for root, dirs, files in os.walk(audio_path):
        for file in files:
            if file.endswith('.mp3'):
                
                audio_file = os.path.join('data', 'matched_files', 'audio', file)
                lrc_file = os.path.join('data', 'matched_files', 'lrc', file.replace('.mp3', '.lrc'))
                ass_file = os.path.join('data', 'matched_files', 'ass', file.replace('.mp3', '.ass'))

                filelist.append([audio_file, lrc_file, ass_file])
